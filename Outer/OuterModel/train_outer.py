...
def nice_prnt(Type_printed, dict_key, train_scores, val_scores, test_scores, crossval_total_num_splits):
	ret = 'Mean  training  {} = {:.5f} +- {:.5f}\n'.format(Type_printed, collect_apply(train_scores, dict_key, np.mean),
	                                                       collect_apply(train_scores, dict_key, np.std) / np.sqrt(
		                                                       crossval_total_num_splits))
	ret += 'Mean validation {} = {:.5f} +- {:.5f}\n'.format(Type_printed, collect_apply(val_scores, dict_key, np.mean),
	                                                        collect_apply(val_scores, dict_key, np.std) / np.sqrt(
		                                                        crossval_total_num_splits))
	ret += 'Mean  testing   {} = {:.5f} +- {:.5f}\n'.format(Type_printed, collect_apply(test_scores, dict_key, np.mean),
	                                                        collect_apply(test_scores, dict_key, np.std) / np.sqrt(
		                                                        crossval_total_num_splits))
	return ret
...
def perform_cross_validation(data, labels,
	...
	if binary_multitask:
		assert num_classes == labels.shape[
			1], 'USER INPUT ERROR: num_classes ({}) and labels in data ({}) are not matching'.format(num_classes,
		                                                                                             labels.shape[1])
	...
	if regression:
		print('Naive baseline (using mean): MSE =', lim(np.mean((labels - labels.mean()) ** 2)), '(RMSE =',
		      train_helper.lim(np.sqrt(np.mean((labels - labels.mean()) ** 2))), ')')
		...
		if not binary_multitask:
			...
			print('Naive baseline (guessing): Accuracy = {}%'.format(
				lim(100 * max([np.mean((labels == v)) for v in range(num_classes)]))))
	...
	for crossval_split_index in range(initial_crossval_index, crossval_total_num_splits):
		...
		traindata, valdata, testdata = utils.cross_validation_split(data, labels,
		                                                            crossval_split_index = crossval_split_index,
		...
		model, (
		train_scores_at_valbest, val_scores_best, test_scores_at_valbest), train_valid_mse_per_epoch, test_predictions = \
		...
		if plot_training_mse:
			train_helper.plot_training_mse_evolution(train_valid_mse_per_epoch,
			                                         ['training set MSE (+regularizer)', 'validation set MSE/accuracy'])
...
def train_model(model, train_data, valid_data, test_data,
	...
	if train:
		...
		for epoch in range(num_epochs):
			...
			if best_valid > normalized_val_mse:
				...
				model_params_at_best_valid = train_helper.get_model_params(
					model)  # kept in RAM (not saved to disk as that is slower)
			if verbose > 0:
		...
		print('Training @', lim(1. / np.mean(times[1:])), 'epochs/sec (',
		      lim(batchsize * len(train_data) / np.mean(times[1:])), 'examples/s)')

	train_helper.set_model_params(model, model_params_at_best_valid)

	training_data_scores = train_helper.eval_metrics_on(train_helper.predict(train_data, model), train_data,
	                                                    regression = regression,
	                                                    Targets_UnNormalization_fn = Targets_UnNormalization_fn)
	validation_data_scores = train_helper.eval_metrics_on(train_helper.predict(valid_data, model), valid_data,
	                                                      regression = regression,
	                                                      Targets_UnNormalization_fn = Targets_UnNormalization_fn)
	test_predictions = train_helper.predict(test_data, model)
	test_data_scores = train_helper.eval_metrics_on(test_predictions, test_data, regression = regression,
	                                                Targets_UnNormalization_fn = Targets_UnNormalization_fn)
	...
	if regression:
		...
		print('train/valid/test set accuracy (best_val): {} / {} / {}'.format(lim(training_data_scores['accuracy']),
		                                                                      lim(validation_data_scores['accuracy']),
		                                                                      lim(test_data_scores['accuracy'])))
		if 'auc' in test_data_scores:
			print('train/valid/test set AUC      (best_val): {} / {} / {}'.format(lim(training_data_scores['auc']),
			                                                                      lim(validation_data_scores['auc']),
			                                                                      lim(test_data_scores['auc'])))

	return model, (training_data_scores, validation_data_scores, test_data_scores), (
	log_train_loss, log_validation_loss), Targets_UnNormalization_fn(test_predictions)
